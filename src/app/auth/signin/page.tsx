...
export default function AuthPage() {
  ...
  const suStrong = suPw.length >= 8 && /[A-Z]/.test(suPw) && /[a-z]/.test(suPw) && /[0-9]/.test(suPw)
          ...
          <div className="px-8 pb-8 pt-6">
            {mode === 'signin' ? (
              <form onSubmit={onSignin} noValidate className="space-y-4">
                ...
                >
                  {siBusy ? (<><span className="mr-2 inline-block animate-spin">⏳</span>{tIn('form.signingIn')}</>) : tIn('form.signInBtn')}
                </button>
              ...
              <form onSubmit={onSignup} noValidate className="space-y-4">
                ...
                <label className="mt-1 inline-flex items-center gap-2 text-sm text-slate-700">
                  ...
                  />
                  {tUp('agree.1')}{' '}
                  <Link href="/terms" className="text-blue-700 hover:underline">{tUp('agree.terms')}</Link>{' '}
                  {tUp('agree.and')}{' '}
                  <Link href="/privacy" className="text-blue-700 hover:underline">{tUp('agree.privacy')}</Link>.
                </label>
                ...
                <button
                  ...
                  className="inline-flex w-full items-center justify-center rounded-xl bg-blue-600 px-4 py-2.5 text-sm font-semibold text-white shadow hover:bg-blue-700 disabled:opacity-60"
                >
                  {suBusy ? (<><span className="mr-2 inline-block animate-spin">⏳</span>{tUp('creating')}</>) : tUp('createBtn')}
                </button>
